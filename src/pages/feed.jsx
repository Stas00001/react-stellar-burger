      ...
      <div className={style.block}>
        ...
        <div className={`${style["block__orders-info"]}`}>
          <div className={`${style.order__status} mb-15`}>
            {orders ? (
              <>
                <OrdersTable done arr={orders} status={"done"} name={"Готовы:"} />
                <OrdersTable
                  arr={orders}
                  status={"pending"}
                  name={"В работе:"}
          ...
          <p className={`${style[`text-shadow`]} text text_type_digits-large mb-15`}>
            {data.total}
          </p>
          <h2 className={`${style} text text_type_main-medium`}>
            Выполнено за сегодня:
          </h2>
          <p className={`${style[`text-shadow`]} text text_type_digits-large`}>
            {data.totalToday}
          </p>
        </div>
      </div>
