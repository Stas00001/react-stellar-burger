...
const OrderInfo = ({ data, modal }) => {
  ...
  const exceptionalIngredients = orderData.ingredients.filter((item, index) => {
  ...
  return (
    <div className={ modal ? `${style.modal}` : `${style.container} mb-10`}>
      ...
      {orderData.status === "done" && (
        <p className={`${style.status} ${style.done} text text_type_main-medium mb-6`}> Выполнен</p>
      )}
      {orderData.status === "created" && (
        <p className={`${style.status} text text_type_main-medium mb-6`}> Создан</p>
      )}
          ...
          return (
            <li className={`${style.item}`} key={info._id}>
              <div className={`${style.ingredient}`}>
                ...
                <p className="text text_type_main-default ml-4">{info.name}</p>
              </div>
