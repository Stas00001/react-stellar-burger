...
const BurgerIngeredienst = (props) => {
    console.log(props.ingeredienst)
    ...
    return (
        ...
        <Tab value="one" active={current === 'one'} onClick = {() => {setCurrent('one'); handleClick(refBun)}}>
            Булки
        </Tab>
        <Tab value="two" active={current === 'two'} onClick = {() => {setCurrent('two'); handleClick(refSauce)}}>
            Соусы
        </Tab>
        <Tab value="three" active={current === 'three'} onClick = {() =>{handleClick(refMain); setCurrent('three')}}>
            Начинки
        </Tab>
        </div>
        <div className={`${burgerIngeredienstStyle.ingeredients__conteiner} custom-scroll`}>
        <h3 ref={refBun} className='text text_type_main-medium pb-6'>Булки</h3>
        <Card onClickCard = {props.onClickCard} data = {arrTypeBun}/>
        <h3 ref = {refSauce} className='text text_type_main-medium pt-10 pb-6'>Соусы</h3>
        <Card increment = {props.increment} count = {props.count} onClickCard = {props.onClickCard} data = {arrTypeSauce}/>
        <h3 ref={refMain} className='text text_type_main-medium pt-10 pb-6'>Начинки</h3>
